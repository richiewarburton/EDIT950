...
BUILD_DIR="$PROJECT_DIR/.build/manual-genuine-regression"
...
CLANG_MODULE_CACHE_PATH="$BUILD_DIR/ModuleCache" swiftc \
  ...
  Tests/GenuineImageRegressionRunner.swift \
  -o "$BUILD_DIR/GenuineImageRegressionRunner" \
  -framework AppKit \
  -framework SwiftUI \
  -framework CoreText \
...
EDIT950_TEST_FONT_DIRECTORY="$PROJECT_DIR/Resources/Fonts" \
  "$BUILD_DIR/GenuineImageRegressionRunner" "$SOURCE_IMAGE" "$AKAIUTIL_PATH"
